...
const SOCIAL: { slug: string; name: string; href: string }[] = [
  { slug: "weibo", name: "微博", href: "#" },
  { slug: "wechat", name: "微信", href: "#" },
          ...
          <div className="flex items-center gap-4">
            {SOCIAL.map((s) => (
              <a
                ...
                aria-label={s.name}
                title={s.name}
                className="inline-flex items-center justify-center h-12 w-12 rounded-full bg-white border border-gray-200 hover:border-gray-400 hover:shadow-md transition-all overflow-hidden"
              >
                <BrandIcon slug={s.slug} size={28} title={s.name} />
              </a>
            ))}
          </div>
